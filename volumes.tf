...
resource "outscale_volumes_link" "volume_link_db" {
  ...
  device_name = "/dev/xvd${var.alphabet[count.index % (var.nb_volume_per_db - 1)]}"
  volume_id   = outscale_volume.volume_db[count.index].id
  vm_id       = outscale_vm.scdsnadb[(count.index - (count.index % (var.nb_volume_per_storage - 1))) / (var.nb_volume_per_db - 1)].id
}
...
resource "outscale_volumes_link" "volume_link_storage" {
  ...
  device_name = "/dev/xvd${var.alphabet[count.index % (var.nb_volume_per_storage - 1)]}"
  volume_id   = outscale_volume.volume_storage[count.index].id
  vm_id       = outscale_vm.scdsnastorage[(count.index - (count.index % (var.nb_volume_per_storage - 1))) / (var.nb_volume_per_storage - 1)].id
}
...
resource "outscale_volumes_link" "volume_link_ws" {
  ...
  device_name = "/dev/xvd${var.alphabet[count.index % (var.nb_volume_per_ws - 1)]}"
  volume_id   = outscale_volume.volume_ws[count.index].id
  vm_id       = outscale_vm.scdsnaws[(count.index - (count.index % (var.nb_volume_per_storage - 1))) / (var.nb_volume_per_ws - 1)].id
}
...
resource "outscale_volumes_link" "volume_link_rancher" {
  ...
  device_name = "/dev/xvd${var.alphabet[count.index % (var.nb_volume_per_rancher - 1)]}"
  volume_id   = outscale_volume.volume_rancher[count.index].id
  vm_id       = outscale_vm.scdsnarancher[(count.index - (count.index % (var.nb_volume_per_storage - 1))) / (var.nb_volume_per_rancher - 1)].id
}
...
resource "outscale_volumes_link" "volume_link_bastion" {
  ...
  device_name = "/dev/xvd${var.alphabet[count.index % (var.nb_volume_per_bastion - 1)]}"
  volume_id   = outscale_volume.volume_bastion[count.index].id
  vm_id       = outscale_vm.scdsnabastion[(count.index - (count.index % (var.nb_volume_per_storage - 1))) / (var.nb_volume_per_bastion - 1)].id
}
